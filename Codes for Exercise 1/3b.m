...
%component)be raveshe hamjavari 8taei ast.BARAYE EJRAYE BARNAME HAME BARNAME
     ...
     for i=2:r+1
         for j=2:c+1
            ...
            if n >= 1
                ...
                if OUTPUT(i+1,j+1)==0;Ua=inf;else Ua = OUTPUT(i+1,j+1); end;%eleman bala
                if OUTPUT(i+1,j-1)==0;Da=inf;else D = OUTPUT(i+1,j-1); end;%eleman paein
                if OUTPUT(i-1,j+1)==0;La=inf;else La = OUTPUT(i-1,j+1); end;%eleman chap
                if OUTPUT(i-1,j-1)==0;Ra=inf;else Ra = OUTPUT(i-1,j-1); end;%eleman rast
                OUTPUT(i,j) = min([Uu Dd Ll Rr Ua Da La Ra]) ;
